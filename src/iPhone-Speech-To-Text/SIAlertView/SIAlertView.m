...
- (id)initWithTitle:(NSString *)title andMessage:(NSString *)message
{
	...
	if (self) {
        ...
        self.transitionStyle = SIAlertViewTransitionStyleBounce;

	}
	return self;
}

- (id)initWithTitle:(NSString *)title message:(NSString *)message delegate:(id)delegate customView: (UIView *)customView cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ...
{
    if (title.length == 0) {
        
        title = NSLocalizedString(@"eBeanstalk", nil);
    }
    
    if (self = [self initWithTitle:title andMessage:message]) {
        
        __weak UIAlertView *selfRef = (UIAlertView *)self;
        ...
        [self addButtonWithTitle:cancelButtonTitle
                         ...
                         handler:^(SIAlertView *alertView) {
                             ...
                             if ([delegate respondsToSelector:@selector(alertView:clickedButtonAtIndex:)]) {
                                 [delegate alertView:selfRef clickedButtonAtIndex:0];
                             }
                             
                             if ([delegate respondsToSelector:@selector(alertView:willDismissWithButtonIndex:)]) {
                                 [delegate alertView:selfRef willDismissWithButtonIndex:0];
                             }
                             
                             if ([delegate respondsToSelector:@selector(alertView:didDismissWithButtonIndex:)]) {
                                 [delegate alertView:selfRef didDismissWithButtonIndex:0];
                             }
                         }];
        ...
        for (NSString *anOtherButtonTitle = otherButtonTitles; anOtherButtonTitle != nil; anOtherButtonTitle = va_arg(args, NSString*)) {
            ...
            [self addButtonWithTitle:anOtherButtonTitle
                             ...
                             handler:^(SIAlertView *alertView) {
                                 if ([delegate respondsToSelector:@selector(alertView:clickedButtonAtIndex:)]) {
                                     [delegate alertView:selfRef clickedButtonAtIndex:buttonIndex];
                                 }
                                 
                                 if ([delegate respondsToSelector:@selector(alertView:willDismissWithButtonIndex:)]) {
                                     [delegate alertView:selfRef willDismissWithButtonIndex:buttonIndex];
                                 }
                                 
                                 if ([delegate respondsToSelector:@selector(alertView:didDismissWithButtonIndex:)]) {
                                     [delegate alertView:selfRef didDismissWithButtonIndex:buttonIndex];
                                 }
                             }];
        }
    }
...
- (void)validateLayout
{
    ...
    if (self.customView) {
        ...
        [self.containerView bringSubviewToFront: self.customView];
    }
...
- (CGFloat)heightForTitleLabel
{
    if (self.titleLabel) {
        CGSize size = [self.title sizeWithFont:self.titleLabel.font
                                   ...
                                   minFontSize:self.titleLabel.minimumFontSize
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < __IPHONE_6_0
            self.titleLabel.minimumFontSize = 0.5;
#else
